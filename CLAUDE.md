...
- 词条类型：首页统计与分类索引展示为**十类**（`course 课程 / concept 概念 / tool 工具(AI 工具箱) / framework 框架 / track 纵深线 / credential 考牌 / job 岗位 / employer 雇主 / channel 渠道 / resource 资源`）；`website 网站` 字段保留可用，但**归入工具大类**展示与统计（不单独计数、不单列分区）。
